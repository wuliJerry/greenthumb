...
(define (test)
  ...
  (send mem store 2 (bv 222 32))
  (assert (bveq (send mem load 2) (bv 222 32)) "mem: load 2 = 222")
  (send mem store 9 (bv 999 32))
  (assert (bveq (send mem load 9) (bv 999 32)) "mem: load 9 = 999")
  (send mem store 9 (bv 0 32))
  (assert (bveq (send mem load 9) (bv 0 32)) "mem: load 9 = 0")
  (pretty-display `(mem ,mem))

  (define mem2 (send mem clone mem))
  (assert (bveq (send mem2 load 9) (bv 0 32)) "mem2: load 9 = 0")
  (assert (term? (send mem2 load 6)) "mem2: load 6 = sym")
  (assert (bveq (send mem2 load 2) (bv 222 32)) "mem2: load 2 = 222")
